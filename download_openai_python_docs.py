#!/usr/bin/env python3
"""
Script to download the OpenAI Python repository and organize its documentation
"""
...
def main():
    print_status("Starting download of OpenAI Python repository")
    
    # Create a unique temporary directory using system temp dir
    unique_id = str(uuid.uuid4())[:8]
    temp_repo = Path(tempfile.gettempdir()) / f"openai-python-temp-{unique_id}"
    ...
    target_dir = Path("openai-python-docs")
    ...
    print_status(f"Copied {len(md_files) - len(excluded_files)} markdown files")
    ...
    # List all directories in the target folder for verification
    print_status("Contents of the target directory:")
    for item in target_dir.glob('*'):
        if item.is_dir():
            if item.name in ["examples", "tests"]:
            ...
            if item.suffix.lower() == ".md" and item.name not in excluded_files:
    ...
    print_status("Successfully completed! Files have been moved to openai-python-docs folder")
